...
public class TransaksiForm {
    ...
    public void updateFields() {
        ...
        transaksiAmountEdit.setText(String.valueOf(transaksiTable.getAmount_transaksi()));
    ...
    public void handleOkButtonAction(ActionEvent event) {
        try {
            ...
            Date date_transaksi = Date.valueOf(localDate); 
            String name_barang = transaksiBarangAdd.getText();
            int amount_transaksi = Integer.parseInt(transaksiAmountEdit.getText());
    ...
    public void handleEditButtonAction(ActionEvent event) {
        try {
            ...
            Date date_transaksi = transaksiTable.getDate_transaksi();
    ...
    public void insertData(Date date_transaksi, String name_barang, int amount_transaksi, String note_transaksi) {
                ...
                try (PreparedStatement checkInsertStmt = conn.prepareStatement(checkSql)) {
                    ...
                    if (!checkRs.next()) {
                        ...
                        try (PreparedStatement insertStmt = conn.prepareStatement(query2)) {
                            ...
                            insertStmt.setInt(4, amount_transaksi);
    ...
    public void updateData(int id_transaksi, Date date_transaksi, String name_barang, int amount_transaksi, String note_transaksi) {
        int id_user = UserSession.getInstance().getLoggedInID();
        String query1 = "SELECT id_barang FROM barang WHERE id_user =? AND name_barang =?";
            ...
            if (rs.next()) {
                ...
                try (PreparedStatement checkNameStmt = conn.prepareStatement(checkNameSql)) {
                    ...
                    if (checkNameRs.next()) {
                        ...
                        String query2 = "UPDATE transaksi SET date_transaksi =?, id_barang =?, amount_transaksi =?, note_transaksi =? WHERE id_user =? AND id_transaksi =?";
                        try (PreparedStatement updateStmt = conn.prepareStatement(query2)) {
                            updateStmt.setDate(1, date_transaksi);
                            updateStmt.setInt(2, id_barang);
                            updateStmt.setInt(3, amount_transaksi);
